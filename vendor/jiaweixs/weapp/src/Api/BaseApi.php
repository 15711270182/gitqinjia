...
class BaseApi
{
	...
	public function sendHttpRequest($url,$url_param = null,$body_param = null,$is_post = true){
		...
		custom_log('sendMessage',$body_param);
		...
		custom_log('sendMessage',print_r($array_data,true));
		if($array_data){
			if(isset($array_data['errcode'])&&$array_data['errcode']!=0&&$array_data['errcode']!=43101&&$array_data['errcode']!=40003){

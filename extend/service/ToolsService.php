...
class ToolsService
{
     ...
     * @param double $money
     * @param int $number
     * 2018年12月21日下午3:53:21 
     * liuxin 285018762@qq.com
     */
    public static  function getBonusList($money,$number){
        if($number==1){
            
            return [$money];
        }
        $redModel = model('Red');
        $avage = bcdiv($money, $number,2);
        $tem_avage = bcdiv($avage,2,2);
        $tem_avage2 = bcdiv($avage,4,2);
        $bonusMin = bcsub($avage, $tem_avage2,2);
        $bonusMax = bcadd($avage, $tem_avage,2);
        $bonus_array = $redModel->getBonus($money,$number,$bonusMax,$bonusMin);
        return $bonus_array;
    } 

...
class NewRecommend extends Model
{
    ...
    public function getLastMatch($uid, $total, $had_id = array())
    {
        ...
        // 根据居住地匹配
        $residence_match_list = $this->getOtherMatch($uid, $total, $request_condition, $had_id, $residence = 1);
...
//        $match_list = $matchDb->limit($num)->order('today_score desc')->fetchSql()->select();
//        echo $match_list;exit();
        if (!empty($match_list)){
...
//        $other_list_match_list = $MatchDb->limit($num)->order('today_score desc')->fetchSql()->select();
//        echo $other_list_match_list;
    ...
    public function tomorrowRecommend($uid, $date)
    {
        ...
        if ($update_info){
            ...
            if ($last_count){ // 判断是否匹配完成,否则补充明日推荐
                ...
                $request_condition = $this->getRequire($age_condition, $height_condition, $eduction);
                // 限制居住地进行匹配
                $residence_match_list = $this->getOtherMatch($uid, $last_count, $request_condition, $had_id, $residence = 1);
                if ($residence_match_list){
                    foreach ($residence_match_list as $key => $value){
                        array_push($had_id, $value['uid']);
                        array_push($update_list, $value['uid']);
     ...
     * @throws ModelNotFoundException
     */
    public function replenishRecommend($uid, $total, $request, $had_id=array())
    {
        $residence_count = 0;
        ...
        while ($residence_count < $total){
            ...
            $other_list = $this->getOtherMatch($uid, $total, $request_condition, $had_id);
            if ($other_list){
     ...
     * @param int $age 默认放宽年龄为 5 岁
     * @return array
     */
    protected function getAge($min_age, $max_age, $age=5)
    {
        $age_arr = [];
